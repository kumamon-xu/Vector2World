...
1. 在干净工作树执行前后端测试和 `build-release.ps1`；RC 为 `UNSIGNED_RC`，正式版为 `UNSIGNED` 且必须从精确 `v<version>` tag 构建。
2. 当前正式发布规则不要求 Authenticode 代码签名；Release notes 和 `release-metadata.json` 必须明示 `UNSIGNED`，并提示 Windows 可能显示“未知发布者”。
...
6. 推送 tag 前运行 `preflight-production.ps1`；tag workflow 会先创建 draft Release，重新下载并验证哈希、版本、commit 和未签名状态后才公开发布。
...
2. 发布修复版时递增 SemVer/build，重新执行双 Windows E2E、升级矩阵、SBOM 与哈希门禁。
